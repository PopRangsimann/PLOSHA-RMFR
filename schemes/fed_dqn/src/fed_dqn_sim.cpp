...
std::vector<double>& QTable::GetQ(const std::vector<double>& state, int num_actions) {
...
bool FedDQNSimulation::LoadDataset(const std::string& csv_path) {
    ...
    while (std::getline(file, line)) {
        ...
        // For sensor data aggregation, use the raw value as ms (lightweight operation).
        t.execution_time = std::stod(token); // 0.001–2.0 ms per reading
        if (t.execution_time < 0.001) t.execution_time = 0.001;
        ...
        if (t.deadline < t.execution_time * 1.1) t.deadline = t.execution_time * 1.1;
    ...
    file.close();
    std::cout << "Loaded " << all_parsed_tasks_.size() << " tasks from dataset." << std::endl;
...
KMeansResult FedDQNSimulation::RunKMeans(int k, int max_iter, double tol) {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        // Update step
        std::vector<std::vector<double>> new_centroids(k, std::vector<double>(2, 0.0));
...
void FedDQNSimulation::AssignPriorities(const KMeansResult& km) {
    ...
    // Compute average execution_time and deadline per cluster (on normalized features)
    // High priority: high exec_time + low deadline (urgent, heavy tasks)
    // Low priority: low exec_time + high deadline (non-urgent, light tasks)
    ...
    // Map cluster index → priority (0=high, 1=medium, 2=low → remap to 2=high, 1=med, 0=low)
...
void FedDQNSimulation::SetHyperparameters(double gamma, double epsilon,
                                            double epsilon_min, double epsilon_decay,
                                            double alpha, int batch_size,
                                            int target_update_freq, int federated_period) {
    gamma_ = gamma;
    epsilon_ = epsilon;
...
std::vector<double> FedDQNSimulation::GetState(const FogNode& node, const Task& task) {
...
int FedDQNSimulation::SelectAction(const FogNode& node, const std::vector<double>& state) {
    ...
    if (dist(rng_) < epsilon_) {
        ...
        return const_cast<QTable&>(node.q_table).GetBestAction(state, num_vms_per_node_);
    }
}

double FedDQNSimulation::ComputeReward(const Task& task, const VirtualMachine& vm) {
    if (task.rejected) {
...
void FedDQNSimulation::TrainFromReplay(FogNode& node) {
    if ((int)node.replay_buffer.size() < batch_size_) return;

    // Sample a batch
    std::uniform_int_distribution<int> dist(0, (int)node.replay_buffer.size() - 1);
    for (int b = 0; b < batch_size_; b++) {
        ...
        // Update Q(s, a)
        node.q_table.Update(exp.state, exp.action, target, alpha_, num_vms_per_node_);
    }
}
...
void FedDQNSimulation::SimulateFailures(std::vector<bool>& failed_nodes) {
    ...
    for (bool f : failed_nodes) if (!f) { all_failed = false; break; }
    if (all_failed) failed_nodes[0] = false;
}
...
void FedDQNSimulation::RecoverFromFailure(int node_id, int& recovery_count,
    ...
    double qtable_cost = static_cast<double>(node.q_table.table.size()) * 0.01; // ms
...
FedDQNMetrics FedDQNSimulation::Run() {
    ...
    int queue_samples = 0;
    double total_task_latency = 0;     // Sum of (completion - arrival) for simulated metric
    ...
    for (int episode = 0; episode < num_episodes_; episode++) {
        ...
        for (auto& task : tasks_) {
            ...
            int fed_interval = std::max(1, federated_period_ * total_tasks / num_episodes_);
    ...
    // Overhead per round scales with (num_nodes * table_entries * entry_transfer_cost)
    double sync_overhead_per_round = num_fog_nodes_ * avg_qtable_size * 0.001; // ms
    ...
    metrics.task_rejection_rate = static_cast<double>(total_rejected) /
                                   static_cast<double>(total_tasks * num_episodes_);
    metrics.avg_queue_utilization = (queue_samples > 0)
        ? total_queue_len / queue_samples / 50.0  // Normalize to max queue depth
        : 0;
