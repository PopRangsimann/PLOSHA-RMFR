...
RecoveryMode RMFREngine::determineRecoveryMode(bool completeness_flag,
                                                 ...
                                                 double reliability,
                                                 double tau_1, double tau_2,
                                                 double tau_3, double tau_f) {
...
int RMFREngine::selectRecoveryCandidate(const std::vector<FogNode>& fog_nodes,
    ...
    for (size_t j = 0; j < fog_nodes.size(); ++j) {
        if (static_cast<int>(j) == failed_fog_id) continue;
        if (fog_nodes[j].isFailed()) continue;
...
RecoveryResult RMFREngine::executeRecovery(CryptoWrapper& crypto,
    ...
    result.mode = determineRecoveryMode(completeness_flag, RU, reliability,
                                        tau_1, tau_2, tau_3, tau_f);

    if (result.mode == RecoveryMode::Normal) {
        auto end = std::chrono::high_resolution_clock::now();
        result.recovery_latency_ms = std::chrono::duration<double, std::milli>(end - start).count();
        return result;
    }

    // Find best candidate
    int target_id = selectRecoveryCandidate(fog_nodes, predictions, failed_fog_id);
    if (target_id < 0) {
    ...
    switch (result.mode) {
        ...
        case RecoveryMode::Failover: {
            ...
            fsm.payload_bytes = sizeof(FogState)
                + fsm.reassigned_sensors.size() * sizeof(int);
...
double RMFREngine::updateReliability(double current_rel, bool success, double completeness_V) {
    double succ_val = success ? 1.0 : 0.0;
    double new_rel = BETA_REL * current_rel
