...
bool FedDQNSimulation::LoadDataset(const std::string& csv_path) {
    ...
    while (std::getline(file, line)) {
        ...
        t.execution_time = std::stod(token) * 1000.0; // Scale to ms-range
        if (t.execution_time < 1.0) t.execution_time = 1.0;
...
void FedDQNSimulation::ParseDatasetToTasks(int num_tasks) {
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        t.rejected = false;
        t.sla_violated = false;
        t.arrival_time = i * 0.1; // Stagger arrivals
        tasks_.push_back(t);
    }
}
...
FedDQNMetrics FedDQNSimulation::Run() {
    ...
    for (int episode = 0; episode < num_episodes_; episode++) {
        ...
        for (int i = 0; i < num_fog_nodes_; i++) {
            if (failed_nodes[i]) {
                RecoverFromFailure(i, recovery_count);
            }
        }
        ...
        for (auto& task : tasks_) {
            ...
            if (scheduling_step % (federated_period_ * total_tasks / num_episodes_) == 0) {
                FederatedAggregate();
            }
    ...
    metrics.aggregation_latency_ms =
        std::chrono::duration<double, std::milli>(t_wall_end - t_wall_start).count() / ep;
